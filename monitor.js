...
const MonitorRecord = {
    ...
    async init(cfg, ethSender, wabSender){
        ...
        backendConfig.wethGroupAddr = config.wabHtlcAddr;
    ...
    getSenderbyChain(chainType){
        return chainType == "ETH"? this.ethSender : this.wabSender;
    },
    async createrSender(ChainType,local=false){
        if(config.hasLocalNode && ChainType=="wab" && local){
    ...
    getDepositRevokeEvent(sender, hashX) {
        let topics = ['0x'+wabUtil.sha3(config.depositOriginRevokeEvent).toString('hex'), null,  hashX];
        let p = pu.promisefy(sender.sendMessage, ['getScEvent', config.originalChainHtlc, topics], sender);
        return p;
    },
    getWithdrawRevokeEvent(sender, hashX) {
        let topics = ['0x'+wabUtil.sha3(config.withdrawOriginRevokeEvent).toString('hex'), null,  hashX];
        let p = pu.promisefy(sender.sendMessage, ['getScEvent', config.wabHtlcAddr, topics], sender);
        return p;
    },
    ...
    async checkOriginLockOnline(record){
        try {
            ...
            if(record.chain == "ETH"){
                ...
                sender = this.getSenderbyChain("wab");
    ...
    async checkXConfirm(record, waitBlocks){
        try {
            let chain = record.chain=='ETH'?"wab":"ETH";
    ...
    async monitorRecord(record){
        ...
        switch(record.status) {
            ...
            case 'sentXConfirming':
                chain = record.chain=='ETH'?"wab":"ETH";
